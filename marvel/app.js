document.addEventListener('DOMContentLoaded', async () => {
  ...
  results.hidden = true;
  // Get your Marvel API key from https://developer.marvel.com
  const apiKey = '';
  ...
  submitButton.addEventListener('click', async () => {
    ...
    const response = await fetch(`https://gateway.marvel.com/v1/public/characters?name=${characterInput}&apikey=${apiKey}`);
    const characterData = (await response.json()).data.results
